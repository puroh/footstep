...
class ProductViewSet(
    ...
    def upload_photo(self, request, pk=None):
        """Upload product photo to S3 or save locally in development."""
        ...
        try:
            photo_url = self._upload_photo(file, product)
        except Exception:
            return Response(
                {
                    "error": "upload_failed",
    ...
    def _upload_photo(self, file, product):
        """Upload product photo to S3 or save locally in development."""
        if settings.AWS_ACCESS_KEY_ID and settings.AWS_SECRET_ACCESS_KEY:
            return self._upload_photo_to_s3(file, product)
        # Development: save to MEDIA_ROOT
        return self._save_photo_locally(file, product)

    def _save_photo_locally(self, file, product):
        """Save file to local media directory and return the URL path."""
        import os

        directory = os.path.join(settings.MEDIA_ROOT, "products", str(product.id))
        ...
        return f"{settings.MEDIA_URL}products/{product.id}/{filename}"

    def _upload_photo_to_s3(self, file, product):
